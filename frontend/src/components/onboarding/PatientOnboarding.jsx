...
import {
  Heart, Phone, QrCode, Smartphone, MapPin, Bell, CheckCircle2, Plus, Trash2,
  Download, Printer, Save, Eye, ArrowRight
} from 'lucide-react';

export const PatientOnboarding = ({ user }) => {
  const navigate = useNavigate();

  // Stepper State (2 to 5 for Patient)
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="text-center space-y-2">
        <h2 className="text-2xl font-extrabold text-white">Patient Emergency Setup</h2>
        <p className="text-xs text-slate-400">Step {step} of 5 — Medical & Contact Onboarding</p>

        <div className="grid grid-cols-5 gap-2 pt-1 max-w-lg mx-auto">
          {[
            { id: 1, label: 'Account' },
            { id: 2, label: 'Medical' },
            { id: 3, label: 'Contacts' },
      ...
      {step === 2 && (
        <form onSubmit={handleStep2Medical} className="glass-panel p-6 rounded-3xl space-y-4 shadow-2xl border border-slate-800">
          <h3 className="text-lg font-bold text-white flex items-center gap-2">
            <Heart className="w-5 h-5 text-rose-500" /> Step 2: Patient Medical Profile
          </h3>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 bg-gradient-to-r from-rose-600 to-amber-600 hover:from-rose-500 hover:to-amber-500 text-white font-bold rounded-xl shadow-lg flex items-center justify-center gap-2 transition-all cursor-pointer"
          >
      ...
      {step === 3 && (
        <form onSubmit={handleStep3Contacts} className="glass-panel p-6 rounded-3xl space-y-4 shadow-2xl border border-slate-800">
          ...
          <button
            ...
            className="w-full py-2.5 bg-slate-800 hover:bg-slate-700 text-cyan-300 rounded-xl text-xs font-bold border border-slate-700 flex items-center justify-center gap-1.5 cursor-pointer"
          >
        ...
        <div className="glass-panel p-6 rounded-3xl space-y-6 shadow-2xl border border-slate-800 text-center">
          ...
          <div className="p-6 bg-slate-900 rounded-3xl border border-slate-800 max-w-sm mx-auto space-y-4 shadow-xl">
            ...
            <div className="text-xs space-y-1">
              <p className="font-bold text-white text-sm">{user?.full_name || 'Patient QR Card'}</p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
            <button
              onClick={() => alert('Viewing QR Card')}
              className="py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold rounded-xl border border-slate-700 flex items-center justify-center gap-1.5 cursor-pointer"
            >
              <Eye className="w-4 h-4 text-cyan-400" /> View
            </button>
            <button
              onClick={() => alert('QR Card downloaded!')}
              className="py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold rounded-xl border border-slate-700 flex items-center justify-center gap-1.5 cursor-pointer"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-left">
            ...
            <div className="p-4 bg-slate-900 rounded-2xl border border-slate-800 space-y-3">
              ...
              <button
                ...
                className={`w-full py-2 text-xs font-bold rounded-xl border transition-all cursor-pointer ${
                  permissions.motion
