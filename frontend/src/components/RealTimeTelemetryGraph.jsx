...
export const RealTimeTelemetryGraph = ({ currentAccel, isMonitoring = false }) => {
  const [dataPoints, setDataPoints] = useState([]);

  useEffect(() => {
    if (isMonitoring && typeof currentAccel === 'number') {
      const now = new Date();
      const timeStr = now.toTimeString().split(' ')[0];

      setDataPoints((prev) => {
        const updated = [...prev, { time: timeStr, accel: +currentAccel.toFixed(2) }];
    ...
    <div className="glass-panel p-5 rounded-3xl border border-slate-800 space-y-3 shadow-2xl">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-2">
          <Activity className={`w-5 h-5 text-cyan-400 ${isMonitoring ? 'animate-pulse' : ''}`} />
          <div>
        ...
        <span className="px-3 py-1 bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 text-xs font-mono font-bold rounded-xl">
          Live Stream: {typeof currentAccel === 'number' ? `${currentAccel} m/s²` : 'N/A'}
        </span>
      </div>
      ...
      {dataPoints.length > 0 ? (
        ...
        <div className="p-8 bg-slate-950/80 rounded-2xl border border-slate-800 text-center space-y-2">
          ...
          <p className="text-xs font-bold text-slate-300">
            Waiting for real hardware motion sensor stream...
          </p>
          <p className="text-[11px] text-slate-500">
            No motion sensor detected on this browser/device. Telemetry waveform updates only when real DeviceMotionEvent data is transmitted.
          </p>
        </div>
      )}
    </div>
